...
fetch(
  ...
  .then((weatherInfo) => {
    ...
    liCity.innerHTML = `<h1>City : ${weatherInfo.name}`;
    ...
    liType.innerHTML =
      '<label>Weather Type :</label><img src= "http://openweathermap.org/img/w/' +
    ...
    const liSunRise = document.createElement("li");
    liSunRise.innerHTML =
      "Sun rise : " +
      unixToNormalTime(weatherInfo.sys.sunrise);
    ul.appendChild(liSunRise);
    const liSunSet = document.createElement("li");
    liSunSet.innerHTML =
      " Sun set : " +
      unixToNormalTime(weatherInfo.sys.sunset);
    ul.appendChild(liSunSet);
